...
export const mapMenuWithKey = (key='', menus=[]) => {
    let result = [];
    ...
    (menus || []).map(item => {
        if(key === item.key) {
            ...
            if(Array.isArray(item.children) && item.children.length) {
                let r = mapMenuWithKey(key, item.children);

                if(r.length) result.push(item);
...
export const getMenuWithKey = (key='', menus=[]) => {
    let result = null;
    ...
    for(let k in menus) {
        ...
        if(k === key) {
            ...
            if(Array.isArray(o.children) && o.children.length) {
                result = getMenuWithKey(key, o.children);
            }
        }
    }
